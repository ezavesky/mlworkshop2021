...
with mlflow.start_run(run_name=run_name):
    ...
    best_score = 1e-6
    for i in range(len(cvModel.avgMetrics)):   # find highest score
        if cvModel.avgMetrics[i] > best_score:
            param_explore = grid[i]
            best_score = cvModel.avgMetrics[i]
            # print(param_explore, cvModel.avgMetrics[i] )
            best_hyperparam = {k.name: param_explore[k] for k in param_explore}
    
    # force train right now
    cf.setParams(**best_hyperparam)
...
# MAGIC 
# MAGIC When you're ready, head on to the next script `2c_EXPERT_PARITY` where we build a model with the settings from experts and begin to head to our final testing grounds.
